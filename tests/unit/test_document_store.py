...
def test_document_path_is_fixed_per_kind_and_format(data_dir):
    assert document_store.document_path("t1", "cv", "docx").name == "cv.docx"
    assert (
        document_store.document_path("t1", "cover_letter", "pdf").name
        == "cover-letter.pdf"
    )
...
def test_unknown_kind_or_format_raises(data_dir):
    with pytest.raises(ValueError, match="unknown document kind"):
        document_store.document_path("t1", "portfolio", "docx")
    with pytest.raises(ValueError, match="unknown document format"):
        document_store.document_path("t1", "cv", "rtf")
...
def test_list_documents_reports_only_what_exists(data_dir):
    ...
    assert document_store.find_document("t1", "cv", "docx") == path


def test_result_roundtrip(data_dir):
    assert document_store.load_result("t1") is None
    document_store.save_result("t1", {"profile_id": "alice", "render_skipped": None})
    result = document_store.load_result("t1")
